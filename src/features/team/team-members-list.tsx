...
export function TeamMembersList({ errorMessage, isError, isLoading, members, onRetry }: TeamMembersListProps) {
  ...
  return <div className="overflow-hidden rounded-xl border border-border bg-card"><div className="hidden overflow-x-auto sm:block"><table className="w-full text-left text-sm"><thead className="border-b border-border bg-muted/40 text-xs font-semibold uppercase tracking-wide text-muted-foreground"><tr><th className="px-5 py-3" scope="col">Member</th><th className="px-5 py-3" scope="col">Roles</th><th className="px-5 py-3" scope="col">Joined</th></tr></thead><tbody className="divide-y divide-border">{members.map((member) => <tr className="align-middle" key={member.membershipId}><td className="max-w-80 px-5 py-4"><MemberIdentity member={member} /></td><td className="px-5 py-4"><MemberRoles member={member} /></td><td className="whitespace-nowrap px-5 py-4 text-muted-foreground">{formatJoinedAt(member.joinedAt)}</td></tr>)}</tbody></table></div><ul className="divide-y divide-border sm:hidden">{members.map((member) => <li className="space-y-3 p-4" key={member.membershipId}><MemberIdentity member={member} /><div className="space-y-1"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Roles</p><MemberRoles member={member} /></div><p className="text-xs text-muted-foreground">Joined {formatJoinedAt(member.joinedAt)}</p></li>)}</ul></div>;
}
