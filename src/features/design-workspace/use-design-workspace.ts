...
import { getDesignDetail, postDesignAction, uploadReview } from "./design-workspace.api";
import { useFinalAssetUpload } from "./final-asset-upload-store";
...
export function useDesignActions(workspaceId: string, researchItemId: string) {
  ...
  const finalAssetUpload = useFinalAssetUpload(
    workspaceId,
    researchItemId,
  );
